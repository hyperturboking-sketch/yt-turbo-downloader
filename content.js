...
if (window.__ytTurboLoaded) { /* already active */ } else {
  window.__ytTurboLoaded = true;

  function getClientVersion() {
    try {
      const meta = document.querySelector('meta[name="msapplication-TileColor"]');
    ...
    return '2.20250722.07.00';
  }

  async function getVideoData(videoId) {
    if (!location.hostname.includes('youtube.com')) return null;

    // Method 1: fetch player API from same-origin (has cookies)
    try {
    ...
    // Method 2: read ytInitialPlayerResponse from page scripts
    try {
      ...
      for (const s of scripts) {
        ...
        if (t.includes('ytInitialPlayerResponse')) {
          const m = t.match(
            /ytInitialPlayerResponse\s*=\s*(\{[\s\S]*?"streamingData"[\s\S]*?\});/
          );
          if (m) {
  ...
  function addDownloadButton() {
    ...
    const actions = document.querySelector(
      '#above-the-fold #menu-container, ytd-watch-metadata #actions, #actions ytd-menu-renderer'
    );
    if (!actions) return;
    ...
    btn.id = 'yt-turbo-btn';
    btn.innerHTML =
      '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg><span>Download</span>';
    ...
    const firstBtn = actions.querySelector(
      'ytd-button-renderer, button, #share-button, #like-button'
    );
